...
test.gam = gam(violations ~ te(latitude, longitude) + s(temperatureHigh) + s(temperatureLow) + Month +day_in_week + s(visibility) + s(uvIndex) + icon, data=data_tidy, family = poisson)
  ...
  server = function(input, output, session){
    ...
    get_predictions = function(){
      Pred.df = data_frame(
        longitude = Chi_grid_xy$X,
        latitude = Chi_grid_xy$Y,
        icon = input$weather_type,
        temperatureHigh = input$range[2],
        temperatureLow = input$range[1],
        uvIndex = input$uvIndex,
        visibility = input$visibility,
        day_in_week = input$dayofweek,
        Month = input$month
        )
      
      p_index = predict(model, newdata = Pred.df, type = "response")
