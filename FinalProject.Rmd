...
weather_temp = readRDS("weather_info_daily.Rds")
weather_temp = read.csv("weather_info_daily.csv") %>% mutate(time = as.Date(time))
...
Chi = st_read("geo_export_2029b708-dae1-4482-ad01-6e4f7381e1f4.shp", quiet = TRUE, stringsAsFactors = FALSE)
...
x_pts = seq(Chi_bb[1] - x_rad, Chi_bb[3] + x_rad, length.out = 1000)
...
ggplot(p, aes(x=X,y=Y,color=violations)) + geom_point() + geom_point(data = redlight_data%>%distinct(latitude,longitude), aes(x = latitude, y = longitude))
