...
<<<<<<< HEAD
  df = data_frame(
    ...
    precipType = ifelse(daily$precipIntensity != 0, daily$precipType, "None"),
...
weather_temp = import("weather_info_daily.Rds")
warnings()
...
p_index = predict(m, newdata= Chi_grid_xy %>% select(-L1) %>% as.matrix())
p = manh_grid_xy %>%
  mutate(
...
poly_sf = p_sf %>%
  group_by(prec) %>%
  summarize() %>%
  st_buffer(dist = (manh_bb[3] - manh_bb[1] ) / 600, joinStyle = 'BEVEL')  %>%
  st_buffer(dist = -(manh_bb[3] - manh_bb[1] ) / 600 / 2.5, joinStyle = 'BEVEL') %>%
  st_intersection(st_geometry(manh_shp))
