...
nearest = function(lat, long, locations){
  distances = pmap(locations %>% select(latitude, longitude),
                   function(latitude, longitude)
    {haversine_dist(lat, long, latitude, longitude)})
  return(locations$segment_id[which.min(unlist(distances))])
}
distinct_camera = redlight_data %>% select(latitude, longitude) %>% distinct()
distinct_segment = Traffic %>% select(segment_id, latitude, longitude) %>% distinct()

min_distances = pmap(distinct_camera,
                      function(latitude, longitude)
                        {nearest(latitude, longitude, distinct_segment)}) %>% 
                 unlist()

#not done, don't run the code below
a = left_join(redlight_data, data.frame(min_distances, distinct_camera))
b = left_join(a, Traffic, by = c("min_distances" = "segment_id", "date"))
```
